...
import com.amitbashan.career_hub.repositories.*;
...
public class AuthService {
    ...
    public Pair<Boolean, Boolean> verifyMutualRecruiterStudentPermission(Long jobApplicationId, String token) {
        Recruiter recruiter = authorizeRecruiter(token);
        Student student = authorizeStudent(token);
        JobApplication jobApplication = jobApplicationRepository.getReferenceById(jobApplicationId);

        boolean recruiterVerified = false;
        boolean studentVerified = false;

        if (recruiter != null) {
            recruiterVerified = recruiter.getId().equals(jobApplication.getJobListing().getRecruiter().getId());
        } else if (student != null) {
            studentVerified = student.getId().equals(jobApplication.getStudent().getId());
        } else {
            return null;
        }

        return Pair.of(recruiterVerified, studentVerified);
    }
}
